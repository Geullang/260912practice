...
@import url('https://fonts.googleapis.com/css2?family=Gowun+Dodum&family=Jua&family=Sunflower:wght@300&display=swap');
...
.st-key-gift_button_wrap div[data-testid="stButton"] button {
    background: linear-gradient(135deg, #6ac0c8 0%, #4e9ed4 100%) !important;
    ...
    box-shadow: 0 8px 20px rgba(88, 157, 192, 0.22) !important;
}
.st-key-gift_button_wrap div[data-testid="stButton"] button:hover {
    background: linear-gradient(135deg, #74c8cf 0%, #58a9dd 100%) !important;
...
def natural_reason_clause(reason: str) -> str:
    """
    이유 문장을 자연스럽게 원인절로 바꿉니다.
    여러 문장으로 적은 경우에는 앞 문장은 연결형으로, 마지막 문장은 원인절로 정리합니다.
    """
    raw = reason.strip()
    raw = raw.replace("\n", " ").strip()
    ...
    def to_connective(s: str) -> str:
        ...
        if s.endswith(("해서", "어서", "아서", "고", "고서", "니까", "면서", "는데")):
            return s
        connect_map = [
            ("완성했어", "완성했고"), ("완성했다", "완성했고"),
            ("성공했어", "성공했고"), ("성공했다", "성공했고"),
...
if st.session_state.page == "input":
    ...
    if "second_quote_idx" not in st.session_state:
        st.markdown('<div class="gift-row"></div>', unsafe_allow_html=True)
        left, otter_col, button_col, right = st.columns([1.0, 0.42, 0.86, 1.0])
        with otter_col:
            st.image("assets/otter_refined.png", width=88)
        with button_col:
            with st.container(key="gift_button_wrap"):
                if st.button(
                    "하나 더 선물할게",
                    ...
                    st.rerun()
        st.markdown(
            '<div class="gift-hint">버튼을 톡 눌러봐</div>',
            unsafe_allow_html=True,
        )
    else:
