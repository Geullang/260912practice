...
# ------------------------------------------------------------
# 자유서술형 입력을 내부 태그로 연결하기 위한 단순 키워드 규칙
# AI가 새 문장을 만들지 않고, 확정된 100개 중 하나만 선택합니다.
# ------------------------------------------------------------
KEYWORD_TO_TAGS = {
    # 기분
    "좋": ["오늘", "희망"],
...
def choose_quote(mood: str, reason: str, wish: str):
    mood_tags = tags_from_text(mood)
    reason_tags = tags_from_text(reason)
    wish_tags = tags_from_text(wish)
    ...
    for idx, quote in enumerate(QUOTES):
        ...
        # 입력에서 뚜렷한 키워드를 못 잡았을 때도
        # 특정 문장 하나만 반복되지 않도록 아주 작은 난수 부여
        score += random.random() * 0.35
        scores.append((score, idx))

    scores.sort(reverse=True)
    top_pool = scores[:7]
    _, chosen_idx = random.choice(top_pool)
    return chosen_idx

def vocative_name(name: str) -> str:
    # 이름 끝 받침 유무를 자동 판별해 '아/야'를 붙입니다.
    clean = name.strip()
    if not clean:
        return ""
    last = clean[-1]
...
with st.form("morning_checkin"):
    ...
    mood = st.text_area(
        "2. 지금 기분은 어떠니?",
        placeholder="예: 조금 불안해 / 기분이 좋아 / 피곤해",
        height=90,
    )
    ...
    wish = st.text_area(
        "4. 바라는 것이 있어?",
        placeholder="예: 마음이 편해졌으면 좋겠어 / 오늘 하루를 잘 보내고 싶어",
        height=100,
    )
...
if submitted:
    if not all([name.strip(), mood.strip(), reason.strip(), wish.strip()]):
        ...
        # 입력값과 선택 결과는 현재 화면에만 사용하며 별도로 저장하지 않습니다.
        st.markdown(
            f"""
        ...
        st.markdown(
            '<div class="small-note">입력한 내용은 별도의 시트나 데이터베이스에 저장하지 않습니다.</div>',
            unsafe_allow_html=True,
        )
