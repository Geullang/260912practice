...
# ------------------------------------------------------------
# 감정 어휘
# 첨부한 감정단어 자료를 바탕으로, 앱에서 활용하기 쉽도록
# 의미가 가까운 감정끼리 세분화해 묶었습니다.
# ------------------------------------------------------------
EMOTION_RULES = {
    "행복·즐거움": [
        "행복", "기쁜", "기뻐", "즐거", "유쾌", "신나", "쾌활", "살맛",
        "환상적", "기분 좋은", "좋은", "경쾌", "활기찬", "상쾌", "산뜻"
    ],
    "평온·편안": [
        "편안", "고요", "담담", "포근", "홀가분", "가벼운", "든든", "시원한", "후련"
    ],
    "감사·감동": [
        "감사", "고마", "감동", "감격", "뭉클", "따뜻", "다정", "애틋", "사랑스러"
    ],
    "기대·설렘": [
        "기대", "바라는", "소망", "간절", "열망", "호기심", "흥분", "짜릿"
    ],
    "만족·자신감": [
        "만족", "뿌듯", "흐뭇", "대견", "기특", "자신 있", "통쾌"
    ],
    "분노·짜증": [
        "분노", "짜증", "화나", "화가", "신경질", "약 오르", "괘씸", "불쾌", "얄미",
        "미운", "원망", "복수심", "끓어오르", "기분이 상"
    ],
    "서운·배신": [
    ...
    "답답·막막": [
        "답답", "암담", "절망", "좌절", "갑갑", "숨 막", "막막"
    ],
    "당황·혼란": [
        "당황", "착잡", "멍한", "어색", "찝찝", "복잡", "혼란", "민망"
    ],
    "부끄러움·죄책감": [
        "부끄", "창피", "죄스", "죄책감", "미안", "민망", "초라", "위축"
    ],
    "외로움·고독": [
        "외롭", "고독", "쓸쓸", "소외", "혼자인 것 같"
    ],
...
SPECIAL_QUOTE_RULES = [
    ...
    },
    {
        ...
        "context": None,
        "preferred": [
            "용기는 두려움에 저항하고 그것을 다스리는 것이지, 두려움이 없는 것이 아니다.",
            "진짜 용기는 두렵지 않은 것이 아니라, 두려운 채로 위험에 맞서는 것이다.",
            "가장 용감한 사람도 잠시 더듬으며 걷는다."
        ]
    },
    {
...
def detect_emotion(text: str):
    return detect_label(text, EMOTION_RULES, "평온·편안")

def detect_context(text: str):
    t = normalize(text)

    # '혼자'는 감정이 아니라 맥락을 함께 보고 판단
...
def choose_quote(mood_text: str, reason_text: str, wish_text: str):
    ...
    return scored[0][1], emotion, context, wish
...
def pastel_class(emotion: str):
    if emotion in ["행복·즐거움", "감사·감동", "기대·설렘", "만족·자신감"]:
        return "pastel-warm"
    if emotion in ["불안·걱정", "두려움", "피로·버거움", "답답·막막"]:
        return "pastel-calm"
...
def build_support_message(emotion, context, wish):
    ...
    if emotion == "감사·감동":
        return (
            "고맙고 따뜻한 마음은 마음속에 오래 남는 힘이 되기도 해. 그 마음을 충분히 느껴도 좋아.",
            "사람과 삶을 따뜻하게 바라보는 마음과 잘 어울려서",
        )

    if emotion == "기대·설렘":
        return (
            "무언가를 기다리는 설렘은 아직 오지 않은 시간을 미리 밝게 만들어 주기도 해. 그 기대를 너무 서둘러 결론 내리지 않아도 좋아.",
            "앞으로 열릴 가능성을 기분 좋게 바라보게 해 주는 뜻이 담겨 있어서",
        )

    if emotion == "만족·자신감":
        return (
            "스스로 만족스럽다고 느끼는 순간은 가볍게 지나치지 않아도 좋아. 네가 해낸 것과 그 과정도 함께 기억해 두면 좋겠다.",
            "지금의 성취와 자신감을 오래 간직하는 데 어울려서",
        )
    ...
    if emotion == "피로·버거움":
        ...
        if context == "수면부족":
            return (
                "잠이 부족한 날에는 집중력과 마음의 여유가 함께 줄어들 수 있어. 오늘은 평소만큼 해내지 못해도 이상한 일이 아니야.",
                "오늘의 상태를 너무 몰아붙이지 않도록 마음을 느슨하게 해 주는 뜻이 담겨 있어서",
            )
        return (
    ...
    if emotion == "당황·혼란":
        return (
            "마음이 복잡할 때는 바로 결론을 내리려고 할수록 더 헷갈릴 수 있어. 지금은 한 가지씩 천천히 정리해도 괜찮아.",
            "모든 것을 한 번에 판단하지 않아도 된다는 뜻이 담겨 있어서",
        )

    if emotion == "부끄러움·죄책감":
        return (
            "부끄럽거나 미안한 마음은 네가 그 일을 중요하게 생각한다는 뜻일 수도 있어. 그 마음에만 머물기보다 다음에 어떻게 해볼지 생각해도 괜찮아.",
            "한 번의 일로 자신 전체를 정하지 않고 다시 움직일 수 있다는 뜻이 담겨 있어서",
        )
    ...
    if emotion == "억울·모욕":
        return (
            "억울한 일을 겪으면 그 장면이 계속 마음에 남을 수 있어. 지금의 화와 상처를 억지로 작게 만들 필요는 없어.",
            "감정에만 머물지 않고 상황을 조금 떨어져 바라보는 데 도움이 될 수 있어서",
        )
