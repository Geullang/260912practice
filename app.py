...
# ------------------------------------------------------------
# 자유서술형 응답을 큰 의미 범주로 연결합니다.
# 4번 바람 > 3번 이유 > 2번 기분 순으로 반영합니다.
# ------------------------------------------------------------
CATEGORY_RULES = {
    "관계": ["친구", "관계", "싸움", "다툼", "화해", "서운", "외롭", "혼자", "부모", "가족", "선생"],
    "학업": ["시험", "성적", "공부", "수능", "입시", "대학", "과제", "발표", "면접"],
    "실수실패": ["실수", "실패", "후회", "망했", "틀렸", "못했", "포기"],
    "불안": ["불안", "걱정", "긴장", "초조", "두렵", "무섭", "떨려"],
    "피로": ["피곤", "지쳐", "졸려", "잠", "아파", "몸", "힘들"],
    "자기신뢰": ["자신감", "자신을 믿", "나를 믿", "잘하고 싶", "용기"],
    "막막함": ["막막", "답답", "모르", "혼란", "정리 안", "갈피"],
    "기쁨": ["좋아", "행복", "신나", "설레", "기대", "즐거"],
    "다시시작": ["다시", "새로", "재도전", "시작하고 싶"],
}
...
def quote_score(quote, mood, reason, wish):
    ...
    # 5) 지나치게 동떨어진 문장을 줄이기 위한 페널티
    # 관계 상황인데 관계/위로 태그가 전혀 없으면 감점
    if "관계" in reason_categories and not any(t in tags for t in ["관계", "위로"]):
        score -= 8
    if "실수실패" in reason_categories and not any(t in tags for t in ["다시 시작", "자기신뢰", "위로"]):
        score -= 8
    if "불안" in reason_categories and not any(t in tags for t in ["위로", "관점 전환", "자기신뢰"]):
        score -= 8
...
def choose_quote(mood: str, reason: str, wish: str):
    ...
    # 키워드를 거의 잡지 못한 경우에는 비교적 범용적인 문장을 선택
    if best_score <= 0:
        for idx, q in enumerate(QUOTES):
            if "오늘" in q["tags"] and ("위로" in q["tags"] or "관점 전환" in q["tags"]):
                return idx
    ...
    if not all([name.strip(), mood.strip(), reason.strip(), wish.strip()]):
        ...
        st.markdown(
            ...
            <div class="result-box">
                ...
                지금 기분이 <strong>{reason.strip()}</strong>한 이유로
                <strong>{mood.strip()}</strong>구나.
                네가 바라는 것이 이루어지기를 바라는 마음으로 문장 하나를 선물할게.
            </div>
            """,
            unsafe_allow_html=True,
        )
